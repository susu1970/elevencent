...
SignUpBtnView::SignUpBtnView(QWidget*parent):QWidget{parent}{
...
SignUpView::SignUpView(QWidget*parent):QWidget{parent}{
  ...
  m_topV->m_label->setText("sign up");    
  ...
  connect(m_btn->m_btn,&QPushButton::clicked,[this](){
    ...
    int txlen=sizeof(TcpProtocol::SignUp)+nameLen+crypwdLen;
    char*txbuf=new char[txlen];
    TcpProtocol::SignUp*su=(TcpProtocol::SignUp*)txbuf;
    memset(su,0,sizeof(TcpProtocol::SignUp));
    su->type=TcpProtocol::Head::TYPE::SIGN_UP;
    su->nameLen=nameLen;
    su->crypwdLen=crypwdLen;
    char*suName=(char*)(su+1);
    memcpy(suName,m_user->m_lineEdit->text().toLatin1().data(),nameLen);
    char*suPwd=suName+nameLen;
    memcpy(suPwd,crypwd.c_str(),crypwdLen);
    su->h2n();
    NetModel::Tx tx(NetModel::STATE_OUT::SIGN_UP,new NetModel::StateCtx(txbuf,txlen,[](NetModel::StateCtx*ctx){
